...
void function InitMainMenuPanel()
{
	...
	var multiplayerHeader = AddComboButtonHeader( comboStruct, headerIndex, "#MULTIPLAYER_ALLCAPS" )
		...
		file.p1lotButton = AddComboButton( comboStruct, headerIndex, buttonIndex++, "Project-P1L0T" )
		Hud_AddEventHandler( file.p1lotButton, UIE_CLICK, OnPlayP1L0TButton_Activate )
		Hud_SetLocked( file.p1lotButton, true )

		file.fdHostButton = AddComboButton( comboStruct, headerIndex, buttonIndex++, "Host FD Private Server" )
		Hud_AddEventHandler( file.fdHostButton, UIE_CLICK, OnHostFDPrivateServer_Activate )
		Hud_SetLocked( file.fdHostButton, true )
	#endif
...
void function TryUnlockNorthstarButton()
{
	...
	if ( file.p1lotButton != null )
		Hud_SetLocked( file.p1lotButton, false )
	if ( file.fdHostButton != null )
		Hud_SetLocked( file.fdHostButton, false )
}

void function OnHostFDPrivateServer_Activate( var button )
{
	if ( Hud_IsLocked( button ) )
		return

	DialogData dialogData
	dialogData.image = $"ui/menu/common/dialog_information"
	dialogData.header = "Frontier Defense (Private Server)"
	dialogData.message = "To host FD with friends, start a local dedicated server, then I'll auto-connect you.\n\n1) In your Titanfall2 folder, run:\n   NorthstarLauncher.exe -dedicated -serverconfig \"R2Northstar/servers/fd_local/ns_server.cfg\"\n2) Come back here and press CONNECT\n\nFriends join using: <your LAN IP>:37015\n\nTip: Use LOBBY TOOLS below to prep/copy join strings (select-all then Ctrl+C)."

	AddDialogButton( dialogData, "CONNECT", HostFD_DoConnectLoop )
	AddDialogButton( dialogData, "LOBBY TOOLS", HostFD_OpenLobbyToolsPrefilled )
	AddDialogButton( dialogData, "CANCEL", null )
	AddDialogFooter( dialogData, "#A_BUTTON_SELECT" )

	OpenDialog( dialogData )
}

void function HostFD_OpenLobbyToolsPrefilled()
{
	// Prefill useful strings for copying/joining.
	// We can't reliably auto-detect LAN/public IP in the UI VM, so we provide a placeholder.
	// Create the cvars if they don't exist on this build (avoids 'ConVar is not valid').
	ClientCommand( "set p1lot_lobby_id \"\"" )
	ClientCommand( "set p1lot_join_id \"\"" )
	SetConVarString( "p1lot_lobby_id", "<your LAN IP>:37015" )
	SetConVarString( "p1lot_join_id", "127.0.0.1:37015" )

	CloseAllDialogs()
	try
	{
		AdvanceMenu( GetMenu( "LobbyIDToolsMenu" ) )
	}
	catch ( e )
	{
		// Fallback: do nothing if the menu isn't available.
	}
}

void function HostFD_DoConnectLoop()
{
	CloseAllDialogs()
	thread HostFD_ConnectLoop()
}

void function HostFD_ConnectLoop()
{
	DialogData dialogData
	dialogData.showSpinner = true
	dialogData.header = "#CONNECTING"
	dialogData.message = "Connecting to local FD server...\n\nIf it fails, make sure you've started a dedicated server (see the previous dialog for the command)."
	AddDialogButton( dialogData, "CANCEL", null )
	AddDialogFooter( dialogData, "#A_BUTTON_SELECT" )
	OpenDialog( dialogData )

	float startTime = Time()
	while ( Time() - startTime < 60.0 )
	{
		// If we successfully connect, we'll leave the main menu.
		if ( GetTopNonDialogMenu() != file.menu )
			return

		ClientCommand( "connect 127.0.0.1:37015" )
		wait 2.0
	}

	CloseAllDialogs()
	DialogData failData
	failData.image = $"ui/menu/common/dialog_error"
	failData.header = "#ERROR"
	failData.message = "Couldn't connect to 127.0.0.1:37015.\n\nMake sure the FD server is running (start_fd_server.bat) and try again."
	AddDialogButton( failData, "#OK", null )
	OpenDialog( failData )
}
...
void function OnPlayP1L0TButton_Activate( var button )
{
	if ( !Hud_IsLocked( button ) )
	{
		SetConVarString( "communities_hostname", "" )
		NSTryAuthWithLocalServer()
		thread TryAuthWithLocalServerP1L0T()
	}
}
...
void function TryAuthWithLocalServerP1L0T()
{
	DialogData dialogData
	dialogData.showSpinner = true
	dialogData.header = "#CONNECTING"
	...
	if ( NSWasAuthSuccessful() )
	{
		NSCompleteAuthWithLocalServer()
		if ( GetConVarString( "mp_gamemode" ) == "solo" )
			SetConVarString( "mp_gamemode", "fd" )

		CloseAllDialogs()

		ClientCommand( "setplaylist fd_easy" )
		ClientCommand( "map mp_thaw" )
	}
	else 
	{
		CloseAllDialogs()
